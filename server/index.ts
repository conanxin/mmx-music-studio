...
if (config.byokLiveConfirmation !== BYOK_LIVE_CONFIRMATION_PHRASE) {
 ...
 if (requestedMode === 'direct-live') {
  ...
  sendJson(res, 200, {
    ...
    stage: 'direct_live_relay_ok',
    generationIntent: byokProviderParams.generationIntent,
    provider: 'minimax',
    audioUrl: directResult.audioUrl,
    downloadUrl: directResult.audioUrl,
    taskId: directResult.taskId,
    model: directResult.model,
    audioResult: {
      available: Boolean(directResult.audioUrl),
      audioUrl: directResult.audioUrl,
      downloadUrl: directResult.audioUrl,
      source: 'provider-url',
      persistence: 'relay-only',
      note: 'Direct-live provider URL is returned for Studio preview/download and is not persisted as a Library track.',
    },
    library: {
      saved: false,
      status: 'not_saved',
      reason: 'direct_live_relay_only_provider_url_not_persisted',
    },
