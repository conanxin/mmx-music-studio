...
### In-flight phase: Phase BYOK-H2C — Dry-Run Pilot Execution (PASS, rolled back) — see [BYOK_H2C_DRY_RUN_PILOT_EVIDENCE_REPORT.md](launch/BYOK_H2C_DRY_RUN_PILOT_EVIDENCE_REPORT.md)

- **Status**: **H2C_DRY_RUN_PILOT_PASS_ROLLED_BACK** (real pilot executed, production rolled back to safe default).
- **Pilot result**: 4/4 testers PASS. 4 unique `[byok-turnstile-ok]` success-path log lines observed. 0 `[byok-turnstile-debug]` during pilot window. 8-pattern leak audit ALL CLEAR. 0 live call, 0 MiniMax call, 0 music, 0 real MiniMax key.
- **H2C deploy**: H2B commit (`baaafd7`) deployed to production. MainPID cycled: 422764 → 428445 (post-H2A) → 437656 (H2C pilot mode) → 441936 (post-rollback).
- **H2C pilot runtime (temporary, since rolled back)**:
  ...
  - Drop-in file: `byok-test.conf` was temporarily rewritten; backup saved to `/tmp/byok-test.conf.h2c-pilot.20260612_221511.bak`.
- **H2C server-side verifications PASS** (in pilot window, before rollback):
  ...
  - **4 real success-path `[byok-turnstile-ok]` log lines** (4 unique requestIds: `byok_8d4ffa2fbe94`, `byok_717b3025da5a`, `byok_d7b73105d73c`, `byok_1a526bf40112`; all `hostname=music.conanxin.com`, all `action=byok-generate`, all `outcome=turnstile_ok`) — proves real-browser Turnstile flow works in production
  - 0 `[byok-turnstile-debug]` during pilot window 22:05:05–22:08:32
...
- **H2C rollback executed**:
  ...
  - `sudo systemctl daemon-reload && sudo systemctl restart mmx-music-studio` → MainPID 441936, active/running
...
- **Music generated**: None.
- **Real user apiKey**: None.
- **H2B success-path logging**: deployed and observed from 4 real testers in production.
- **H2A dry-run pilot planning**: PASS_WITH_KNOWN_VALIDATION_EXCEPTION (predecessor).
...
- **Smoke test**: `scripts/byok-h2c-final-evidence-smoke-test.sh` (asserts evidence doc contains all 4 requestIds, action metadata, lessons, and links).
...
### In-flight phase: Phase BYOK-H3B-DRILL — Dry-Run Rollback Drill Evidence (current focus)

- **Status**: DRY-RUN DRILL EVIDENCE ONLY. This is a no-live dry-run rollback drill. H3B-DRILL does not execute BYOK live generation, does not call MiniMax, does not generate music, does not open BYOK to a broad public audience.
- **Env change in this drill**: Safe-default rewrite only (idempotent; no posture change). The drill re-wrote `byok-test.conf` to safe-default values, removed the prior `turnstile-debug.conf` and `byok-h2c-dry-run.conf` drop-ins, and restarted the service. The real Turnstile site/secret drop-in (`turnstile-real.conf`, mode 600) was NOT touched.
- **Drill start**: 2026-06-13T00:30:12+08:00 (DRILL_DATE=20260613).
- **MainPID before**: 441936 → after restart: 503163.
- **Real MiniMax call**: None.
- **Music generated**: None (`dailyGenerationUsed=0` post-drill).
- **Real user apiKey**: None (only `sk-FAKE-H3B-ROLLBACK-DRILL-0000000000000000000000` placeholder used).
...
- **Verifications recorded**: `PUBLIC_BYOK_ENABLED=false`, `BYOK_DRY_RUN_ONLY=true`, `BYOK_DIRECT_LIVE_ENABLED=false`, `TURNSTILE_DEBUG_REDACTED=` (empty/unset), `TURNSTILE_SITE_KEY_CONFIGURED=true`, `TURNSTILE_SECRET_KEY_CONFIGURED=true` (boolean only, secret NOT printed). `/api/health` shows `publicByokEnabled=false`, `byokEnabled=false`, `hasServerKey=false`, `byokKeyStorage=memory`, `dailyGenerationUsed=0`, `realApiAttemptsUsed=0`. `/api/generate/byok` with fake key returns `code: "byok_generation_disabled"` (server did not enter Turnstile / MiniMax / music). `/ops` and `/api/status` return `HTTP/2 302` redirect to Cloudflare Access login with `www-authenticate: Cloudflare-Access`.
...
- **Music generated**: None.
- **Real user apiKey**: None.
- **H1 valid-token browser E2E**: PASS (predecessor phase, closed).
...
- **No production env change.** **No code change.** **No release tag.** The v0.4.31-alpha tag stays at `ee6a8a1`.
...
- Approval phrase has been received.
- T1–T5 are confirmed anonymous slots; real tester identities remain outside the repo.
- Pilot window: 2026-06-13T04:45:04+08:00 → 2026-06-13T05:15:04+08:00 (Asia/Shanghai)
- Live execution still requires a separate H3B execution instruction document.
...
- This phase is documentation only; it does not execute live generation.
...
* Rollback verified: post-rollback POST returned `code: byok_generation_disabled`
