...
> ⚠️ **重要**：导入路径是 `apps/weapp/`（不是 `apps/weapp/dist/`）
> `project.config.json` 已配置 `"miniprogramRoot": "./dist/"`，工具会自动找到编译产物。

1. 打开微信开发者工具
2. 点击「导入项目」
3. 项目路径：`/home/ubuntu/projects/mmx-music-studio/apps/weapp/`
4. AppID：测试号（touristappid）或自己的小程序 AppID
5. 点击「确定」

**开发阶段设置（必须）**：
- 详情 → 本地设置 → 勾选「不校验合法域名、web-view（业务域名）、TLS 版本以及 HTTPS 证书」
- 因为当前 API Base `http://118.195.129.137:8787` 是 HTTP IP
...
Phase 3B ✅ 已完成接入自托管 server Mock API：

- `apps/weapp/src/config/api.ts` — API Base 配置，默认 `http://118.195.129.137:8787`
- `apps/weapp/src/adapters/request.ts` — 封装 getHealth、generateTrack、listTracks
- 设置页可配置 API Base + 测试连接
- 创作页调用 `/api/generate`（Server Mock，不消耗额度）
- 作品库调用 `/api/tracks`，失败时 fallback 本地 Mock
- `scripts/weapp-api-smoke-test.sh` 验证通过

微信开发者工具调试 HTTP IP 时需勾选"不校验合法域名"。
...
| 阶段 | 操作 | 状态 |
|------|------|------|
| Phase 3B | 接入 server API（mock 模式） | ✅ 完成 |
| Phase 3C | 实现音频播放和下载 | ✅ 完成 |
| Phase 3D | 微信开发者工具真机预览 | ✅ 完成 |
| Phase 3E | HTTPS 域名 + 合法域名配置准备 | ✅ 完成 |
| Phase 3F | 真实生成受控测试 | 📋 |
...
- `docs/WEAPP_DEVTOOLS_IMPORT.md` — 开发者工具导入指南（Phase 3D ✅）
- `docs/WEAPP_REAL_DEVICE_CHECKLIST.md` — 真机测试清单（Phase 3D ✅）
- `docs/WEAPP_TROUBLESHOOTING.md` — 问题排查（Phase 3D ✅）
- `docs/WEAPP_DOMAIN_HTTPS_GUIDE.md` — HTTPS 域名配置总览（Phase 3E ✅）
- `docs/CADDY_DEPLOYMENT.md` — Caddy 部署指南（Phase 3E ✅）
- `docs/NGINX_DEPLOYMENT.md` — Nginx 部署指南（Phase 3E ✅）