...
| **BYOK API** | ✅（用户自备 key） | 用户自付 | ⚠️ 需警告 | 建议 | 自托管、无注册、BYOK |
| **Private Real** | ✅ | ✅（服务器 key） | ❌ 不建议 | 建议 | 个人自用真实生成 |
...
## 4. BYOK API（自托管 BYOK 模式）

### 用途
- 自托管部署，用户自带 MiniMax Token Plan Key
- 无需注册，BYOK（Bring Your Own Key）
- 用户消耗自己的额度，站长不承担费用
- 适合开源社区、个人开发者
...
# BYOK 配置
BYOK_ENABLED=true
SERVER_KEY_FALLBACK=false  # 强烈建议：禁止自动回退到服务器 key

# 服务器
HOST=127.0.0.1  # 反向代理后面
PORT=8787
MUSIC_OUTPUT_DIR=./storage/tracks
MINIMAX_REGION=cn
```
...
- `x-minimax-api-key` 只通过 HTTP header 传递，不在 URL 或 body 中
- Key 只存在服务器内存 Map，TTL 30 分钟，任务结束后立即删除
- 不写入 manifest / audit log 原文 / server logs
- `SERVER_KEY_FALLBACK=false` 防止服务器 key 被意外消耗
- 前端 key 只存在 React state，刷新页面即清除
...
| `byokEnabled=true && backend=api` | BYOK API |
| `realGenerationEnabled=true && backend=cli` | 真实生成 |
| `realGenerationEnabled=true && backend=api && !byokEnabled` | API 实验 |
